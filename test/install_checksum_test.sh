...
binary=oh-my-terminal
tag=v9.9.9-test
valid_contents='verified fixture'
tampered_contents='tampered fixture'

work=$(mktemp -d "${TMPDIR:-/tmp}/oh-my-terminal-install-test.XXXXXX")
...
case "$url" in
  https://api.github.com/repos/DevNewbie1826/oh-my-terminal/releases/latest)
    [ -z "$output" ] || exit 2
    printf '{"tag_name":"%s"}\n' "${MOCK_RELEASE_TAG:?}"
    ;;
  "https://github.com/DevNewbie1826/oh-my-terminal/releases/download/${MOCK_EXPECT_TAG:?}/"*)
    [ -n "$output" ] || exit 2
    asset=${url##*/}
    case "$asset" in
      oh-my-terminal_linux_amd64.tar.gz|oh-my-terminal_darwin_amd64.tar.gz|checksums.txt)
